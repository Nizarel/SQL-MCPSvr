...
public partial class Tools
{
    ...
    {
        using var activity = _telemetry.StartToolActivity(nameof(DescribeTable));
        var stopwatch = System.Diagnostics.Stopwatch.StartNew();
        
        try
        {
            activity?.SetTag("table.name", name);
            
            // Check cache first
            var cacheKey = CacheKeys.TableSchema(name);
            var cachedSchema = await _cacheService.GetAsync<Dictionary<string, object>>(cacheKey);
            if (cachedSchema != null)
            {
                _telemetry.RecordCacheHit("describe_table", cacheKey);
                _logger.LogDebug("Returning cached table schema for: {TableName}", name);
                _telemetry.RecordToolExecution(nameof(DescribeTable), stopwatch.Elapsed.TotalSeconds, true);
                return new DbOperationResult(success: true, data: cachedSchema);
            }
            
            _telemetry.RecordCacheMiss("describe_table", cacheKey);
            ...
            try
            {
                using (conn)
                {
                    ...
                    using (var cmd = new SqlCommand(TableInfoQuery, conn))
                    {
                        ...
                        {
                            _telemetry.RecordToolExecution(nameof(DescribeTable), stopwatch.Elapsed.TotalSeconds, false);
                    ...
                    }                    // Cache the result for 35 minutes (table schema changes infrequently)
                    await _cacheService.SetTableSchemaAsync(cacheKey, result);
                    _logger.LogDebug("Cached table schema for: {TableName} for 35 minutes", name);
            ...
            {
                _telemetry.RecordError("database_error", nameof(DescribeTable));
                _telemetry.RecordToolExecution(nameof(DescribeTable), stopwatch.Elapsed.TotalSeconds, false);
        ...
        {
            _telemetry.RecordError("tool_error", nameof(DescribeTable));
            _telemetry.RecordToolExecution(nameof(DescribeTable), stopwatch.Elapsed.TotalSeconds, false);
            _logger.LogError(ex, "DescribeTable failed: {Message}", ex.Message);
            return new DbOperationResult(success: false, error: ex.Message);
        }
